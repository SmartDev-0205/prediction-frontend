...
import { toShortStr } from "../util/utils";
const SideOpenPane = ({
    isRight = true,
    ...
    const { makeBet, isLoading, isSpinLoading, isWriteLoading, isApproveLoading, refetch } = useBet({ poolId, amount, side: isRight ? 2 : 1 })
    ...
    return (
        <div className={`flex flex-col ${isRight ? 'items-end' : 'items-start'}`}>
            {(isLoading || isWriteLoading || isApproveLoading || isSpinLoading) && <div className="w-screen h-screen fixed top-0 left-0 bg-black opacity-50 z-40 flex justify-center items-center">
                <ReactLoading type={"spinningBubbles"} height={100} width={100} />
            </div>}
            {status === STATUS.POOL_OPEN ?
                <div className={`flex flex-col ${isRight ? 'items-end' : 'items-start'}`}>
                    <div className="flex flex-wrap relative">
                        <img alt="loading" src={isRight ? rightdiv : leftdiv} className="w-[270px] 2xl:w-[320px] min-h-[400px]" />
                        <div className={`absolute w-full h-full flex flex-wrap flex-col gap-2 pt-4 ${isRight ? 'pr-4 pl-20' : 'pl-4 pr-16'} pb-16`}>
                            {isRight ? downBet?.bets?.map((item, index) =>
                                <div className="flex items-center justify-center h-[35px] w-full text-white border border-[#5757b1]"><span>{toShortStr(downBet.addresses[index])}:</span>{formatEther(item)}</div>
                            )
                                : upBet?.bets?.map((item, index) =>
                                    <div className="flex items-center justify-center h-[35px] w-full text-white border border-[#5757b1]"><span className="font-bold text-gray-300">{toShortStr(upBet.addresses[index])}:</span>{formatEther(item)}</div>
                                )}
                        </div>
                    </div>
                ...
                <div className={`flex flex-col relative ${isRight ? 'items-end' : 'items-start'}`}>
                    <img alt="loading" className="lg:block hidden max-w-[400px] h-[600px]" src={isRight ? users_play_area_down : users_play_area_up} />
